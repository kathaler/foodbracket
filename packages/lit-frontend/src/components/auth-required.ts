...
export class AuthRequiredElement extends App.View {
  @state()
  ...
  _handleRegister(event: SubmitEvent) {
    ...
    const data = new FormData(form);
    const request = new FormDataRequest(data);
    const username = (request.json as { username: string }).username;

    this._dispatchNewProfile(username);
  ...
  _dispatchNewProfile(username: string) {
